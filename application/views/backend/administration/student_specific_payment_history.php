...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#class_id').on('change', function() {
			...
			$.ajax({
				url: '<?php echo site_url('administration/get_sections_for_ssph/');?>' + class_id
			}).done(function(response) {
				$('#section_holder').html(response);
				$('#section_id').select2();
				var section_id = $('#section_id').val();
				$.ajax({
					url: '<?php echo site_url('administration/get_students_for_ssph/');?>' + class_id + '/' + section_id
				}).done(function(response) {
		...
		$('#find').on('click', function() {
			...
			$.ajax({
				url: '<?php echo site_url('administration/get_payment_history_for_ssph/');?>' + student_id
			}).done(function(response) {
				$('#data').html(response);
			});
		});
